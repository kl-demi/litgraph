...
| `Trait` nodes from the Trait Ontology | 1,587 non-obsolete TO terms (release 2026-01-14). Keyed on TO's native id verbatim (e.g. `TO:0000276`), `source_db="TO"`. `to.obo` also carries imported PO/CHEBI/GO/PATO terms, which are filtered out by id prefix — loading them would key `Trait` nodes on ids `Pathway` (GO) and `Compound` (CHEBI) already model. Live in the `rice` database as of 2026-08-03. | `spokebio/ingest/trait_ontology.py`, `spokebio/pipeline.py::run_to_ingest` |
| `ASSOCIATED_WITH` edges (Gene -> Trait) | 33,512 edges over 6,680 rice genes and 585 traits, from Oryzabase's curated gene list. The trait-side counterpart to the GAF pathway edges, and what makes a trait-centric query possible on a rice corpus — there is no rice equivalent of the human disease-linkage sources (DisGeNET, OMIM). Resolves genes via `build_locus_identifier_crosswalk` (81.7%); MATCHes `Trait` rather than creating it, so annotations to TO terms obsoleted since Oryzabase wrote them are counted and dropped rather than minting nameless nodes (646 edges, 1.9%). Live in the `rice` database as of 2026-08-03: 4,672 genes now carry both a trait and a pathway edge. | `spokebio/ingest/oryzabase.py`, `spokebio/pipeline.py::run_oryzabase_ingest` |
| Rice gene `MENTIONS` from a dictionary pass | 16,003 Paper->Gene edges over 51,166 rice papers, from Oryzabase symbols/synonyms resolved through `build_locus_identifier_crosswalk`. Exists because PubTator3 finds a gene in only 7.4% of rice papers and gets the species right 4.9% of the time. Conservative by default: only structurally rice-specific forms (RAP/MSU locus ids, `Os`-prefixed symbols) plus a hand-audited allowlist; a permissive tier is available behind `include_unaudited` for LLM-disambiguation candidates. Edges stamped `source="oryzabase-gazetteer"`. Live in production 2026-08-03 -- lifted the trait-centric query from 104 to 6,498 supporting papers. | `spokebio/ingest/gene_gazetteer.py`, `spokebio/pipeline.py::run_gazetteer_mentions` |
...
`ingest/{pubtator,go,gene_crosswalk,reactome,chebi_mesh_crosswalk,gaf,trait_ontology,oryzabase,gene_gazetteer}.py`)
